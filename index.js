...
import { CommunityPoll, EmbedBase, SentenceService } from './classes';
import bot from './bot';
...
const postInit = async function () {
    ...
    await (async function registerCommands() {
        const cmds = await bot.pog_guild.commands.set(bot.commands.map(({ run, ...data }) => data))
        ...
        await bot.pog_guild.commands.permissions.set({ 
