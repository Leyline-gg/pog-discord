...
class selfrole extends Command {
    ...
    async run({intr}) {
        ...
        try {
            ...
            const avail_roles = (await bot.pog_guild.roles.fetch()).filter(r => bot.config.self_roles.includes(r.id));
            ...
            bot.intrReply({intr, embed: new EmbedBase({
                fields: [
                    ...(!!add.length ? [{
                        name: 'Roles Added',
                        value: add.map(role => bot.pog_guild.roles.resolve(role).toString()).join('\n'),
                    }] : []),
                    ...(!!rem.length ? [{
                        name: 'Roles Removed',
                        value: rem.map(role => bot.pog_guild.roles.resolve(role).toString()).join('\n'),
                    }] : []),
                ],
            }), components: [], content: '\u200b'})
        } catch (err) {
