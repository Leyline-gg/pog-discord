...
export class ReactionCollectorBase {
	...
	setupApprovedCollector({duration = this.REACTION_WINDOW * 3600 * 1000} = {}) {
		...
		this.collector = msg.createReactionCollector({ 
			...
			try {
				...
				if(!(await Firebase.isUserConnectedToPog(user.id))) 
					this.handleUnconnectedAccount(user, {
						dm: `You reacted to the [${this.media_type}](${msg.url} 'click to view message') posted by ${bot.formatUser(msg.author)} in <#${msg.channel.id}>, but because you have not connected your Discord & Pog accounts, I couldn't award you any GP!
							[Click here](${bot.connection_tutorial} 'How to connect your accounts') to view the account connection tutorial`,
						log: `${bot.formatUser(user)} reacted to the [${this.media_type}](${msg.url} 'click to view message') posted by ${bot.formatUser(msg.author)} in <#${msg.channel.id}>, but I did not award them any GP because they have not connected their Pog & Discord accounts`,
					});
	...
	/**
	 * Process for dealing with a Discord user that lacks a connected Pog account
	 ...
	 */
	 handleUnconnectedAccount(user, {dm, log} = {}) {
		...
		bot.sendDM({user, embed: new EmbedBase(bot, {
			fields: [
				{
					name: `❌ You need to Connect Your Pog & Discord accounts!`,
					value: dm,
				},
			],	
		}).Error()});
	...
	async awardApprovalGP({user, approver, pog}) {
		...
		await Firebase.awardPoints(await Firebase.getPogUID(user.id), this.APPROVAL_GP, {
			category: pog,
	...
	async awardReactionGP({user, pog=`Discord <a href="${this.msg.url}">Moral Support</a>`}) {
		...
		await Firebase.awardPoints(await Firebase.getPogUID(user.id), this.REACTION_GP, {
			category: pog,
	...
	async awardAuthorReactionGP({user, pog}) {
		const { bot, msg } = this;
		//new user reacted, award GP
		await Firebase.awardPoints(await Firebase.getPogUID(msg.author.id), this.REACTION_GP, {
			category: pog,
