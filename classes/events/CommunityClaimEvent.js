...
export class CommunityClaimEvent {
     ...
     * @param {PogUser} params.lluser User that will receive the NFT
            ...
            fields: [
                {
                    ...
                    value: `You have been awarded a(n) ${nft.rarity.toLowerCase()} **${nft.name}**!
                        Check it out on your [Pog profile](https://pog.gg/profile)!`,
                },
            ],	
        })});
        return true;
    ...
    createCollector(msg=this.msg) {
        ...
        msg.createMessageComponentCollector({
            ...
            if(!(await Firebase.isUserConnectedToPog(user.id)))
                return bot.intrReply({
                    intr,
                    embed: new EmbedBase(bot).ErrorDesc('You have not connected your Pog & Discord accounts!'),
                    ephemeral: true,
                });
            ...
            //const lluser = await new PogUser(await Firebase.getPogUID(user.id));
