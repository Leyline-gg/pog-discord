...
export class PogBot extends Client {
    ...
    get pog_guild() {
        return this.guilds.resolve(this.config.pog_guild_id);
    }
     ...
     * @param {Object} args Destructured arguments. `options` will be passed to `Pogthis.intrReply()` as params
    ...
    // ----- Other Methods -----
    /**
     * Checks if a user has mod permissions on the Pog server.
     ...
     */
    checkMod(uid) {
        return this.pog_guild.members.cache.get(uid).roles.cache.some(r => this.config.mod_roles.includes(r.id));
    }

    /**
     * Checks if a user has admin permissions on the Pog server.
     ...
     */
    checkAdmin(uid) {
        return this.pog_guild.members.cache.get(uid).permissions.has('ADMINISTRATOR');
    }
